 ...
 */
export async function sendDonationFollowUp(paymentIntentId) {
  try {
    // Retrieve the payment intent from the database
    const paymentIntent = await DAO.initialpaymentmethod.getPaymentIntent(paymentIntentId);
    if (!paymentIntent) {
