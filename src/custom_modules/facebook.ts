...
export const postExternalDonation = async (donation: Donation, fundraiser: Fundraiser) => {
  const token = await fetchToken();

  const response = await fetch(`https://graph.facebook.com/${fundraiser.id}/external_donations`, {
    method: "POST",
    headers: {
    ...
    body: JSON.stringify({
      ...
      currency: "NOK",
      donation_id_hash: createHash("md5").update(donation.id.toString()).digest("hex"),
      donor_id_hash: createHash("md5").update(donation.donorId.toString()).digest("hex"),
