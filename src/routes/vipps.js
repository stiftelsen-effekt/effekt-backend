...
router.put(
  ...
  async (req, res, next) => {
    try {
      ...
      if (response)
        await mail.sendVippsAgreementChange(agreementCode, "UNPAUSED");
...
router.post("/agreement/notify/change", jsonBody, async (req, res, next) => {
  try {
    ...
    const response = await mail.sendVippsAgreementChange(
      agreementCode,
      change,
      newValue
    );
...
router.post("/agreement/report/problem", jsonBody, async (req, res, next) => {
  try {
    ...
    const response = await mail.sendVippsProblemReport(
      senderUrl,
      donorEmail,
      donorMessage,
      agreement
