...
router.post("/register/payment", async (req, res, next) => {
  try {
    ...
    if (!donorID) {
      donorID = await DAO.donors.add({ email, full_name, newsletter: false });
...
router.post("/register/campaign", authMiddleware.isAdmin, async (req, res, next) => {
  try {
...
router.post("/register/donations", authMiddleware.isAdmin, async (req, res, next) => {
    ...
    try {
      ...
      if (!donorID) {
        ...
        if (donors && donors.length === 1) {
          donorID = donors[0].ID;
          console.log(`Found donorID ${donorID} by exact match on full name ${fullName}`);
        }
      }
      ...
      if (!donorID) {
        donorID = await DAO.donors.add({ email, full_name: fullName, newsletter: false });
      ...
      const campaignOrgShares = await DAO.facebook.getFacebookCampaignOrgShares(campaignID);
      ...
      distribution.forEach((org, i) => (distTotalPercent += parseFloat(String(org.share))));
      ...
      if (Math.round(distTotalPercent) != 100) {
        ...
        distribution[0].share = String(
          Math.round(parseFloat(String(distribution[0].share)) + 100 - distTotalPercent * 100) /
            100,
        );
      }
      ...
      if (registeredDonation) {
        ...
        let registered = await DAO.facebook.getRegistededFacebookDonationByDonorID(donorID);
      ...
      if (typeof KID !== "string") {
        ...
        invalidTransactions.push({
          transaction: donationInfo,
          reason:
            "Donation" + String(externalRef) + ": KID " + String(KID) + " not valid or not found. ",
        });
        continue;
      }
